...
function Dashboard() {
  ...
  // 1. Fetch data from your new Node backend when the page loads
  useEffect(() => {
    fetch('http://localhost:9000/api/topics')
      .then(res => res.json())
      .then(data => {
        // Map MongoDB's '_id' to standard 'id' for the UI
  ...
  // 2. Calculate progress based on the real database data
  useEffect(() => {
    if (topics.length === 0) {
      setProgress(0);
      return;
  ...
  // 3. Send the updated status back to the Node database
  const handleStatusChange = async (id, newStatus) => {
    ...
    // Send the PATCH request to save it permanently in MongoDB
    try {
        ...
        <div className="space-y-4">
          ...
          {topics.length === 0 && (
            <div className="text-center py-8 text-gray-400">
              Loading your curriculum from the cloud...
            </div>
          )}
        </div>
      </div>
